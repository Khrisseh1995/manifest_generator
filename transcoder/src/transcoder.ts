...
export default class Transcoder {
    ...
    private async uploadToS3(key: string, body: string, objectToSend: any) {
        if (!key || !body) {
            return;
        }
    ...
    private async postToManifestGenerator(manifestInformation: expressParam): Promise<void> {
        ...
        return axios.post("http://manifest-generator/create_manifest", manifestInformation);
    ...
    public async startTranscode(): Promise<void> {
        ...
        ffmpeg.stderr.on('data', async data => {
            ...
            if (isMatch) {
                ...
                promisesArray.push(this.uploadToS3(chunk.file_name, `../streams/${chunk.file_name}`, objectToSend));
